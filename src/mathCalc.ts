...
(function(app,exposeYourself:boolean){
  ...
  var dpsChar:Crusader|null = null;
   ...
   * @return {World}
   */
  // this could stand to have methods moved to World.prototype.X = function...
  function World(id, name, spots) {
    ...
    this.whatsAdjacent = function (spot:number):number[] {
   ...
   * @param {number} spot2
   */
  var getAreInSameColumn = (currentWorld, spot1, spot2) => currentWorld != null && getIsValidSpotNumber(currentWorld, spot1) && getIsValidSpotNumber(currentWorld, spot2) && currentWorld.columnNum(spot1) == currentWorld.columnNum(spot2);
  /**
   *
   * @param {World} currentWorld
   * @param {number} front
  ...
  /*: World */
  front,
  maybeBehind) => currentWorld != null && getIsValidSpotNumber(currentWorld, front) && getIsValidSpotNumber(currentWorld, maybeBehind) && currentWorld.columnNum(front) == currentWorld.columnNum(maybeBehind) + 1;
  ...
  */
  ...
  kyle.calculate = function () {
    ...
    for (var i = 0; i < adjacent.length; i++) {
      ...
      if (adjCruId) {
        ...
        if (adjCru.tags.includes("leprechaun")) {
          leprechaunAdjacent = true;
        }
        if (adjCru.tags.includes("animal")) {
          animalAdjacent = true;
        }
        if (dpsChar && adjCruId == dpsChar.id) {
          dpsSmashed = true;
        }
        if (currentWorld.columnNum(spot) > currentWorld.columnNum(adjCruSpot)) {
          numBehind += 1;
        }
        if (currentWorld.columnNum(spot) < currentWorld.columnNum(adjCruSpot)) {
          numAhead += 1;
    ...
    if (dpsChar && dpsSmashed) {
      (dpsChar as any).smashed = true;
    } else if (dpsChar) {
      (dpsChar as any).smashed = false;
    }
  };
  ...
  henry.calculate = function () {
    ...
    if (henry == dpsChar) {
      ...
      for (var i = 0; i < adjacent.length && noOneBehind; i++) {
        ...
        if (currentWorld.columnNum(spot) == currentWorld.columnNum(adjCruSpot) + 1) {
          noOneBehind = false;
        }
      }
  ...
  grandmora.calculate = function() {
    ...
    var dpsSpot = dpsChar && getDpsSpot(app.formationIds, dpsChar);
    if (dpsSpot != null && currentWorld.columnNum(grandmora.spot) == currentWorld.columnNum(dpsSpot) - 1) {
  ...
  princess.calculate = function () {
    ...
    if (dpsChar && !dpsChar.tags.includes('event')) {
  ...
  jason.calculate = function () {
    ...
    if (currentWorld.columnNum(spot) == currentWorld.maxColumn && jason == dpsChar && numAttacking > 0) {
  ...
  drizzle.calculate = function () {
    ...
    if (groklok == dpsChar && currentWorld.columnNum(spot) == currentWorld.columnNum(groklokSpot)) {
      drizzle.globalDps *= 5;
    }
  ...
  khouri.calculate = function () {
    ...
    if (dpsChar && dpsChar.tags.includes('magic')) {
    ...
    var karenSpot = getCrusaderSpot(app.formationIds, karen.id);
    if (karen == dpsChar && currentWorld.columnNum(khouriSpot) != currentWorld.columnNum(karenSpot) - 1) {
      karen.effects += 1;
    }
  };
  ...
  brogon.calculate = function () {
    ...
    var karenSpot = getCrusaderSpot(app.formationIds, karen.id);
    if (karen == dpsChar && !adjacent.includes(karenSpot)) {
      karen.effects += 1;
    }
  ...
  halfblood.calculate = function () {
    ...
    if (dpsChar && dpsChar.tags.includes('male')) {
  ...
  gryphon.calculate = function () {
    ...
    // bonus
    if (dpsChar && karen == dpsChar && getIsBehind(karenSpot, grphyonSpot) === false) {
  ...
  helper.calculate = function () {
    ...
    if (dpsChar && dpsChar.tags.includes('leprechaun')) {
    ...
    var karenSpot = getCrusaderSpot(app.formationIds, karen.id);
    if (karen == dpsChar && currentWorld.columnNum(karenSpot) != currentWorld.columnNum(helperSpot)) {
      karen.effects += 1;
    }
  };
  ...
  val.calculate = function () {
    ...
    if (dpsChar && dpsChar.tags.includes('royal')) {
  ...
  cindy.calculate = function () {
    ...
    if (dpsChar && dpsChar.tags.includes('orc')) {
