...
(function(app,exposeYourself:boolean){
  ...
  class World {
    ...
    constructor(id:number, name:string, spots:number) {
      var _this = this;
      this.id = id;
      this.name = name;
      this.spots = spots;
    ...
    whatsAdjacent(spot:number | null | undefined):number[] {
    ...
    getFilledColumnSpots(column:number, tag?:Tag) {
      var count = 0;
      app
        .formationIds
        .map((cruId, i) => {
    ...
    findDistances (spot1:number):number[] {
      var visited:boolean[] = [];
      var distance:number[] = [];
      var adjacent:number[] = [];
      var current = spot1;
    ...
    setAdjacent(spot:number,adjacentArray:number[]):void{
  ...
  /*: World */
  front,
  maybeBehind) => currentWorld != null && getIsValidSpotNumber(currentWorld, front) && getIsValidSpotNumber(currentWorld, maybeBehind) && currentWorld.getColumnNum(front) == currentWorld.getColumnNum(maybeBehind) + 1;
  ...
  */
  ...
  var worldsWake : World = app.worldsWake = new World(1, "World's Wake", 10);
  (() => {
    var x = null;
    (worldsWake as World).layout = [
      [
        0, x, x, x
      ],
      [
  ...
  (() => {
    ...
    adjacents.map(spotWithAdj => gardeners.setAdjacent(spotWithAdj[0] as number, spotWithAdj[1] as number[]));
  })();
  ...
  //Set Up Formation
  var currentWorld : World = worldsWake;
