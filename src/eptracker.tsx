...
(function(app,exposeYourself:boolean){
  ...
  (TagsTd as any).displayName = 'TagsTd';

  var CruTagRowFormationEpBox = props =>
        props.isEpMode && props.isOwned ? (
          <div className="ep"><app.TextInputUnc type="number" min="0" readonly={app.getIsUrlLoaded()} onChange={props.onEpChange} className={["medium"]} value={props.enchantmentPoints} />
            {props.effectiveEp && !Number.isNaN(props.effectiveEp) ? (<div className="sharedEp">Shared:{props.effectiveEp}</div>) : null}
          </div>)
          : null;
  (CruTagRowFormationEpBox as any).displayName = 'CruTagRowFormationEpBox';
  ...
  var CruTagRowFormation = props =>(
    props.mode ==="mine" && props.isFormationMode ?
          ...
          : null);
  (CruTagRowFormation as any).displayName = 'CruTagRowFormation';
  ...
  (CruTagRowSlotGear as any).displayName = "CruTagRowSlotGear";
  ...
  (CruTagRowGear as any).displayName = "CruTagRowGear";
  ...
  var getSortClasses = value => (sortStates as any[]).find(val => val.value === value).classes;
  var getNextSortValue = value => (sortStates as any[]).find(val => val.value === value).next;
  ...
  interface CruTagGridProps{
    updateSave(newData:object) : void
    slotSort?:Sorting
    epSort?:Sorting
    nameSort?:Sorting
    filterOwned:number
  ...
  class CruTagGrid extends React.Component<CruTagGridProps,CruTagGridState> {
    constructor(props){
      super(props);
    ...
    onGearChange(cruId,slot,gearTypeIndex, selectV){
      var stateMods : CruTagGridState = { crusaderGear:{}, epFilter:undefined};
      if(!this.props.crusaderGear){
        stateMods.crusaderGear = {};
      } else {
        var merged = app.copyObject(this.props.crusaderGear);
    ...
    render(){
      ...
      if(isMineMode){
        var epFilterComponent = (
          <select
              value={this.state.epFilter}
              onChange={e => this.setState({epFilter:app.inspect(e.target.value,"epFilter.val")})}>
            <option>None</option>
  ...
  class CruApp extends React.Component {
    ...
    render(){
      ...
      var importArea = app.getIsUrlLoaded() ? null : (<Exporter
                    ...
                    onUpdateClick={() => !app.getIsUrlLoaded() ?  this.setState({lastRead:cruTagGrid.readOrDefault(undefined)}): null}
                    clipper={clipper}
