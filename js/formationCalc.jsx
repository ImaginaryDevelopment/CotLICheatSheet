...
(app => {
    ...
    var getFormationDiags = (worldId, formation) => formation
        .map((cruId, i) => {
    ...
    var makeHeroSelect = (formation, slotNumber, onFormationChange) => {
        ...
        formation.filter(f => f != null && f != "0").find(f => getCrusader(f).slot == cru.slot) == null
            || (selectedCru && (selectedCru.id == cru.id || selectedCru.slot == cru.slot)));
        return (<div>{slotNumber}
            <app.HeroSelect dontSort={true} crusaders={availableCrusaders} onHeroChange={changeFormation(slotNumber, onFormationChange)} selectedHeroId={selectedCru && selectedCru.id}/>
                        {selectedCru != null ?
            <app.GearBox cru={selectedCru} cruGearQ={cruGearQ}/>
            : null}
        </div>);
    };
    var dpsSelector = (formation, onDpsChange, dpsCruId) => (<app.HeroSelect crusaders={app.jsonData.crusaders.filter(cru => formation.filter(f => f != null).findIndex(fId => fId == cru.id) >= 0)} onHeroChange={cruId => {
        if (cruId == "0")
            cruId = null;
    ...
    var makeWorldRenderer = (props, worldId, slotLayout) => {
        ...
        var myMakeTrSlots = (row, slots) => (<tr key={row} title={"row" + row}>
                    {slots.map((slot, i) => {
            if (slot != null) {
        ...
        return (<div>
                        ...
                        <table>
                            <thead></thead>
                            <tbody>
                                {slotLayout.map((slots, row) => (myMakeTrSlots(row, slots)))}
                        </tbody>
                        </table>
                        <div>
                            ...
                            <select value={props.selectedSave} onChange={e => props.selectedSaveChange(e.target.value)}>
                                <option key="_">None</option>
                                {(app.inspect(props.saveNames, "saveNames")).map(sn => (<option key={sn} value={sn}>{sn}</option>))}
                            </select>
    ...
    class FormationCalc extends React.Component {
        ...
        render() {
            ...
            if (world != null && world.layout != null) {
                ...
                formationComponent =
                    (<WorldComponent slotLayout={world.layout} spots={world.spots} worldId={this.state.selectedWorldId} formation={formationIds} dpsCruId={dpsCruId} onFormationChange={this.onFormationChange} onDpsChange={this.onDpsChange} onClearClick={this.clearFormation.bind(this)} 
                    // for saving
                    nextSaveName={app.inspect(this.state.nextSaveName, "nextSaveName")} enableSave={this.state.enableSave !== false} onSaveFormationClick={() => this.saveFormation(this.state.nextSaveName, formationIds, dpsCruId)} onSaveNameChange={d => this.setState({ nextSaveName: d })} 
                    // for loading
                    selectedSave={this.state.selectedSave} selectedSaveChange={e => this.setState({ selectedSave: e })} saveNames={app.inspect(this.state.saveNames || [], "saveNames")} onLoadFormationClick={() => this.loadFormation(this.state.selectedSave)}/>);
            }
            else {
            ...
            var kaineXpComponent = kaine && formationIds.includes(kaine.id) && rarity > 3
            ...
            return (<div>
                ...
                <select value={this.state.selectedWorldId} onChange={e => {
...
//# sourceMappingURL=formationCalc.jsx.map