...
(function (app, exposeYourself) {
    ...
    var HeroGameData = props => {
        ...
        var heroLIs = props.mappedHeroes ? props.mappedHeroes.map(h => (<li data-key={h.HeroId} key={h.HeroId}>{JSON.stringify(h)}</li>)) : [];
        var gearLIs = gear.map(l => (<li data-key={l.lootId} key={l.lootId}>{JSON.stringify(l)}</li>));
        ...
        var talentLIs = talents.map(t => (<li data-key={t.talentId} key={t.talentId}>{JSON.stringify(t)}</li>));
                ...
                switch (pFieldName) {
                    ...
                    case "crusaderGear":
                        ...
                        if (!(gear != null))
                            return;
                        r.gear = {};
                        Object.keys(gear).map(slotKey => r.gear[slotKey] = gear[slotKey]);
                        break;
                    default:
