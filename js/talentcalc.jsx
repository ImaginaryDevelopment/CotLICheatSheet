...
(function (app, exposeYourself) {
    ...
    app.HeroSelect = props => {
        ...
        var options = crusaders
            .map(cru => <option key={cru.id} className={cru.tags.includes("dps") ? "dps" : ""} value={cru.id}>{cru.displayName} ({cru.slot})</option>);
        ...
        return (<select title={title} value={props.selectedHeroId || "0"} className={selectedCrusader && selectedCrusader.tags && selectedCrusader.tags.includes("dps") ? "dps" : ""} onChange={e => props.onHeroChange(e.target.value)}>
    ...
    app.NonDpsTalent = props => (<td title={props.name}><app.TextInputUnc value={props.value} type="number" min={props.min} max={props.max} onChange={props.onChange}/></td>);
    ...
    app.Inputs = props => {
        ...
        var rows = sortedDpsTalentNames.map((name, i) => {
            ...
            switch (name) {
                ...
                case "rideTheStorm":
                    return makeTalentTrArray(tdInfo, i, (<td>Current Storm Rider:{currentStormRider}</td>), (<td>Next Storm Rider: {nextStormRider}</td>));
                case "stormsBuilding":
                    return makeTalentTrArray(tdInfo, i);
                default:
        ...
        return (<table>
            ...
            <tbody>
                ...
                <tr data-row={15}>
                    ...
                    <td><app.RaritySelect gearTypes={props.referenceData.gearTypes} includeGoldens={true} onChange={val => props.onStormRiderPercentageChange(getStormRiderPercentageFromRarity(val))}/></td>
                </tr>
                <tr data-row={16}/>
                <tr data-row={17}>
    ...
    var talentCalc = props => {
        ...
        var tic = {
            ...
            dpsInfo: {
                cru: talentSelectedCrusader,
                ep: talentSelectedCrusader && talentSelectedCrusader.mainDpsEP || 0,
                slotEp: talentSelectedCrusader && talentSelectedCrusader.dpsSlotEP,
                epics: talentSelectedCrusader && talentSelectedCrusader.mainDpsEpics,
        ...
        return (<app.Inputs {...props} tic={tic} crusaders={crusaders} talents={props.referenceData.talents} onCritChanceChange={val => (props.changeSaveState({ critChance: app.inspect(+val || 0, 'changeSaveState crit') }))} idols={props.saved.idols} onIdolsChange={val => props.changeSaveState({ idols: val })} sortTalents={props.sortTalents} onSortTalentChange={props.onSortTalentsChange} onCooldownCommonChange={val => props.changeSaveState({ cooldownCommon: +val || 0 })} onCooldownUncommonChange={val => props.changeSaveState({ cooldownUncommon: +val || 0 })} onCooldownRareChange={val => props.changeSaveState({ cooldownRare: +val || 0 })} onCooldownEpicChange={val => props.changeSaveState({ cooldownEpic: +val || 0 })} selectedHeroId={typeof (props.saved.talentCalcHeroId) === "string" ? props.saved.talentCalcHeroId : undefined} onHeroChange={val => props.changeSaveState({ talentCalcHeroId: val })} onStormRiderPercentageChange={createChangeHandler("stormRiderPercentage")} nonDpsTalents={ntt}/>);
    };
...
//# sourceMappingURL=talentcalc.jsx.map